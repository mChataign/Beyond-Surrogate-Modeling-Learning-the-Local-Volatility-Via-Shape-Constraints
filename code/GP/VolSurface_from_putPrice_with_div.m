function [T, M, IV] = VolSurface_from_putPrice_with_div(S0, r_int, div_int, K, T, PutPrice)
%**************************************************************************
...
for i = 1:length(ImpliedVol)
    try
          ImpliedVol(i) = fzero(@objfcn, [0 10], options, ...
                                 S0, K(i), T(i), r_int(i), div_int(i), PutPrice(i));
                catch
          ImpliedVol(i) = NaN;
    end 
end
...
function P = BlackScholesPrice_Put(S0,K,T,r_int, div_int,sigma)

    d1=(log(S0./K)+r_int-div_int+(0.5.*sigma.^2)*T)./(sigma.*sqrt(T));
    d2=(log(S0./K)+r_int-div_int-(0.5.*sigma.^2)*T)./(sigma.*sqrt(T));
    
    discountFactor = exp(-r_int);
    forwardFactor = exp(-div_int);
    
    P = K.*discountFactor.*normcdf(-d2)-S0.*forwardFactor.*normcdf(-d1);
...
function delta = objfcn(volatility, S0, K, T, r_int, div_int, PutPrice)

    BSprice = BlackScholesPrice_Put(S0, K, T, r_int, div_int, volatility);
